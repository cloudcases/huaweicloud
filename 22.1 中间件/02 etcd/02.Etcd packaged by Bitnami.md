...
```console
$ docker run -it --name Etcd bitnami/etcd:3.5.1

$ docker run -it --env ALLOW_NONE_AUTHENTICATION=yes --name Etcd bitnami/etcd:3.5.1

$ docker run -d --env ALLOW_NONE_AUTHENTICATION=yes --name Etcd bitnami/etcd:3.5.1
```
...
- All Bitnami images available in Docker Hub are signed with [Docker Content Trust (DCT)](https://docs.docker.com/engine/security/trust/content_trust/). You can use to verify the integrity of the images.`DOCKER_CONTENT_TRUST=1`
...
## Supported tags and respective links`Dockerfile`
...
To run this application you need [Docker Engine](https://www.docker.com/products/docker-engine) >= . [Docker Compose](https://docs.docker.com/compose/) is recommended with a version or later.`1.10.0``1.6.0`
...
To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/bitnami/etcd/tags/) in the Docker Hub Registry.
...
Use the argument to the command to attach the Etcd container to the network.`--network app-tier``docker run``app-tier`
...
When not specified, Docker Compose automatically sets up a new network and attaches all deployed services to that network. However, we will explicitly define a new network named . In this example we assume that you want to connect to the Etcd server from your own custom application image which is identified in the following snippet by the service name .`bridge``app-tier``myapp`
...
> **IMPORTANT**:
>
> 1. Please update the placeholder in the above snippet with your application image`YOUR_APPLICATION_IMAGE`
> 2. In your application container, use the hostname to connect to the Etcd server`etcd`
...
For example if you want to modify the flag , you will need to set the environment variable.`--my-flag``ETCD_MY_FLAG`

The previous rule applies to all [etcd flags](https://coreos.com/etcd/docs/latest/op-guide/configuration.html).

> Note: by default the environment variable is set to . Modify this environment variable to use a different API version.`ETCDCTL_API``3`
...
- Arbitrary user ID(s) when running the container with a non-privileged user are not supported (only UID is allowed).`1001`
